...
const BRAND = {
  ...
  serviceCity: "Melbourne",
  rating: "4.6",
};
```

Change `serviceCity` and the hero badge and footer contact line update
automatically — you only edit it here, once.
...
All prices in the site are in AUD (`$`), set in `js/data.js` under
...
correctly localized to the Melbourne market — they are not real quotes
from your business. Before you go live, search `js/data.js` for `basePrice` and
...
automatically): `basePrice: 69` displays as `$69`.
...
| File | Used for |
|---|---|
| `logo-wide.png` | The full wide logo banner — shown in the header on tablet/desktop (≥640px) and always in the footer |
| `favicon-32.png`, `favicon-180.png`, `favicon-512.png` | Square crop of the logo (the car graphic) — used as the browser tab icon and as the compact logo mark in the header on phones (<640px), where the wide banner would crowd out the WhatsApp/Book Now/menu buttons |
...
There is intentionally **no payment form, no card fields, no name/phone/
email fields, and no terms checkbox** anywhere on this site — it's built
to be as fast as possible on a phone. Booking works like this:

1. Person picks a vehicle type, service, add-ons, date, and a **written**
   preferred time (a plain text field, not a fixed time-slot picker — see
   below) on `pages/booking.html`.
2. The last step just shows an order summary and an optional notes field
   (parking instructions, gate code, etc.) — nothing else to fill in.
3. Tapping **"Send Booking via WhatsApp"** builds a message with every
   detail they picked (vehicle, service, add-ons, date, time, address,
   notes, estimated total) and opens WhatsApp with that message
   pre-filled, ready for them to hit send.
4. You receive the message on WhatsApp — their name and number are
   already visible to you there automatically — and confirm the exact
   time and arrange payment directly with the customer from there.

You don't need to configure anything for this to work beyond setting
`WHATSAPP_NUMBER` (and `WHATSAPP_DISPLAY`, the human-readable version
shown next to "Book Now" in the navbar) correctly — see section 1. If you
want to change the wording of the generated message, it's built in
`js/booking.js` inside the `buildBookingMessage()` function — each line
is plain text you can edit.

**Why a written time instead of a slot picker?** It's simpler and faster
on a phone — the person just types "2pm" or "after 4" or whatever suits
them, and you confirm the exact time back on WhatsApp. No fake
"available slots" to maintain.
...
```html
<title>Mob Car Detailing — Mobile Car Detailing in Melbourne</title>
<meta name="description" content="Mob Car Detailing — professional mobile car detailing serving Melbourne...">
```
...
- Every service already gets its own URL (`service.html?service=ceramic-coating`,
  ...
  data — good for ranking on searches like "ceramic coating Melbourne".
...
- **Pricing is in AUD** for the Melbourne market. The actual numbers
  are placeholder estimates — see section 1.
