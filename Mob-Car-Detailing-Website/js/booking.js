...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateConversionBarForStep(step){
    ...
    btn.textContent = step === 4 ? "Send via WhatsApp" : "Continue";
  }
  ...
  // STEP 3 — Date & Time
  // ------------------------------------------------------------------
  const ALL_SLOTS = ["9:00 AM", "11:00 AM", "1:00 PM", "2:00 PM", "4:00 PM"];
  const MAX_DAYS_OUT = 45;

  function isDayFullyBooked(dateKey){ return hashStr(dateKey) % 9 === 0; }

  function slotsForDate(dateKey){
    const hash = hashStr(dateKey);
    const bookedCount = hash % 3;
    const booked = new Set();
    for(let i = 0; i < bookedCount; i++) booked.add((hash + i) % ALL_SLOTS.length);
    return ALL_SLOTS.map((s, i) => ({ label: s, available: !booked.has(i) }));
  }
  ...
  function buildCalendar(){
    ...
    document.querySelectorAll("[data-date]").forEach(btn => {
      ...
      btn.addEventListener("click", () => {
        ...
        selectedTime = null;
        setBookingDraft({ date: selectedDateKey, time: null });
        buildCalendar();
        renderTimeSlotsForSelected();
        updateStep3NextState();
      });
    });
  }

  function renderTimeSlotsForSelected(){
    const wrap = document.getElementById("timeSlots");
    if(!selectedDateKey){
      wrap.innerHTML = `<p class="text-muted" style="font-size:14px;">Select a date to see available times.</p>`;
      return;
    }
    const slots = slotsForDate(selectedDateKey);
    const availableCount = slots.filter(s => s.available).length;
    const badgeClass = availableCount <= 2 ? "badge-warning" : "badge-success";
    wrap.innerHTML = `
      <div class="avail-badge"><span class="badge ${badgeClass}">${availableCount} slot${availableCount === 1 ? "" : "s"} left on ${prettyDate(selectedDateKey)}</span></div>
      <div class="slot-grid">
        ${slots.map(s => `<button class="slot-pill ${selectedTime === s.label ? "is-selected" : ""}" ${!s.available ? "disabled" : ""} data-time="${s.label}">${s.label}</button>`).join("")}
      </div>`;
  ...
  function updateStep3NextState(){
    document.getElementById("toStep4").disabled = !(selectedDateKey && selectedTime);
  }
  ...
  function validateStep4(){
    ...
    toggleFieldError(name, !name.value.trim());
    if(!name.value.trim()) ok = false;

    const phoneOk = phone.value.replace(/\D/g, "").length >= 7;
    toggleFieldError(phone, !phoneOk);
    if(!phoneOk) ok = false;

    // Email is optional here (WhatsApp is the primary contact channel) —
    // only validate format if something was typed in.
    const emailVal = email.value.trim();
    const emailOk = emailVal === "" || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailVal);
    toggleFieldError(email, !emailOk);
    if(!emailOk) ok = false;

    const termsRow = terms.closest(".checkbox-row");
    const termsErrEl = showElError("termsError", !terms.checked, "Please accept the terms to continue.");
    if(!terms.checked){ termsRow.after(termsErrEl); ok = false; } else { termsErrEl.remove?.(); }
  ...
  document.getElementById("addToCalendarBtn").addEventListener("click", () => {
    ...
    const ics = [
      ...
      `DESCRIPTION:Mobile detailing appointment at ${getBookingDraft().address || ""}`,
  ...
  document.getElementById("referBtn").addEventListener("click", async function(){
    const shareData = { title: BRAND.name, text: "Get your car detailed by Mob Car Detailing — mobile car detailing in Lahore:", url: window.location.origin };
    if(navigator.share){
